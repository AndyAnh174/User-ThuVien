...
-- Chạy với DV_OWNER sau khi restart database
-- ============================================
...
BEGIN
    ...
    -- ADMIN_USER - Participant (can access)
    DVSYS.DBMS_MACADM.ADD_AUTH_TO_REALM(
        realm_name   => 'LIBRARY_REALM',
        grantee      => 'ADMIN_USER',
        rule_set_name => NULL,
...
EXCEPTION 
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('Auth Error: ' || SQLERRM);
END;
/
...
SELECT REALM_NAME, ENABLED FROM DVSYS.DBA_DV_REALM WHERE REALM_NAME = 'LIBRARY_REALM';
